...
def generate_explanations(features: dict, risk_output: dict):
    ...
    # max_days_overdue drives the heuristic blend (risk_scoring.py) but isn't
    # part of the trained model's feature vector, so SHAP never sees it.
    # Surface it explicitly so a critically overdue customer's explanation
    # doesn't silently omit the reason a human would call out first.
    days_overdue = features.get("max_days_overdue", 0)
    if days_overdue > 0:
        amount = features.get("overdue_amount", 0)
        explanations.append({
            "factor": "max_days_overdue",
            "reason": f"{int(features.get('overdue_item_count', 0))} unpaid invoice(s) overdue by up to {int(days_overdue)} days (₹{amount:,.0f} outstanding)",
            "impact": "high" if days_overdue > 30 else "medium",
        })
    ...
    explanations.extend(get_shap_explanations(features_with_level))
    return explanations
