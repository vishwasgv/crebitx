...
import { recordCollectionOutcome } from "@/app/actions/operating-intelligence"
...
export function WhatsAppDialog({ customerName, amount, phone, customerId }: WhatsAppDialogProps) {
  ...
  const handleSend = async () => {
    ...
    if (customerId) {
      await recordCollectionOutcome({
        customerId,
        channel: "WHATSAPP",
        tone: tone === "ease" ? "GENTLE" : tone === "medium" ? "FIRM" : "URGENT",
        outcomeStatus: "SENT",
        note: getMessage(),
      })
    }
      ...
      <DialogContent className="sm:max-w-[500px] rounded-[2rem] border-[rgba(190,200,202,0.2)] bg-[#fef8f3] p-8">
        ...
        <div className="py-6 space-y-8">
          <RadioGroup defaultValue="ease" onValueChange={setTone} className="grid grid-cols-3 gap-3">
            {[
              { id: "ease", label: "Gentle", icon: "G" },
              { id: "medium", label: "Firm", icon: "F" },
              { id: "warm", label: "Urgent", icon: "U" },
            ].map((t) => (
              <div key={t.id}>
                ...
                >
                  <span className="text-xl mb-1 font-black text-[#005259]">{t.icon}</span>
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center px-1">
              ...
              {tone !== "custom" && (
                <button
                  onClick={() => {
                    setTone("custom")
                    setCustomMessage(templates[tone])
                  }}
            ...
            <div className="bg-white p-5 rounded-2xl border border-[rgba(190,200,202,0.2)] shadow-inner">
              {tone === "custom" ? (
                ...
                <p className="text-sm font-medium text-[#3f494a] leading-relaxed italic">&ldquo;{getMessage()}&rdquo;</p>
              )}
            </div>
          </div>
        </div>
